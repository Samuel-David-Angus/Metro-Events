...
function OrganizerRequests() {
        ...
        axios.get('http://localhost:3001/Requests')
            .then(
                response => {
                    ...
                    const pendings = req.filter(item => item.type === "organizer" && item.status === "pending");
    ...
    return (
      <div className= "OrganizerRequests">
        ...
        {users && organizerRequests.map(
            (req, index) => (<OrganizerRequestCard key={index}  req={req} users={users} setReload={setReload} reload={reload}/>)
        )}
      </div>
    );
  }
  ...
  function OrganizerRequestCard({ req, users, setReload, reload }) {
    ...
    const handleApprove = () => {
        ...
        axios.patch(`http://localhost:3001/Users/${req.user_id}`, {type: "organizer"})
        .then(
            response => {
